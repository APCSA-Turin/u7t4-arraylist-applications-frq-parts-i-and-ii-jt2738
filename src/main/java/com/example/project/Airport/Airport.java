...
public class Airport  {
    ...
    public int updateFlights() {
        int bootedPassengers=0;
        for(int i=0; i<allFlights.size();i++){
            double lessThanCap= allFlights.get(i).getCapacity()*.20;
            if(allFlights.get(i).getNumPassengers()<lessThanCap){
                bootedPassengers+= allFlights.get(i).getNumPassengers();
                allFlights.remove(i);
                i--;
            }
        }
        return bootedPassengers;
    }
}